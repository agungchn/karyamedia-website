          ...
          {results.map((p) => (
            ...
            >
              <div className="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center shrink-0 overflow-hidden">
                <Image src={p.image || "/images/logo-karyamedia.png"} alt={`${p.name} - ${categories.find((c) => c.id === p.categoryId)?.subcategories.find((s) => s.id === p.subcategoryId)?.name || categories.find((c) => c.id === p.categoryId)?.name || "Souvenir"} Karyamedia Souvenir`} width={40} height={40} className="object-cover w-full h-full" />
              </div>
