...
function generateDescriptiveSlug(name, existingSlugs) {
  ...
  let cleanName = name
    .replace(/^Medali Custom\s*/i, "")
    .replace(/^Medali\s*/i, "")
    .trim()

  // Generate base slug
  let baseSlug = "medali-" + slugify(cleanName)

  // If empty or too short, use fallback
  if (baseSlug.length < 10) {
    baseSlug = "medali-custom"
  }
...
for (const product of data) {
  if (product.subcategoryId !== "md") continue
...
console.log(`\nTotal produk medali custom yang diubah slug-nya: ${updateCount}\n`)
